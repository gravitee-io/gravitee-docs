---
redirect_from:
  - /am/current/am_devguide_protocols_oauth2_overview.html
---
= OAuth 2.0
...
[NOTE]
====
OAuth 2.0 also supports the use of <<refresh tokens>> to obtain new access tokens.

AM provides a mechanism for defining additional types. See link:../../extension-grants/introduction.html[Extension grants^] for more information.
...
Refresh tokens are used to get a new access token, prompting the client application to renew access to protected resources without displaying a login page to the resource owner.
