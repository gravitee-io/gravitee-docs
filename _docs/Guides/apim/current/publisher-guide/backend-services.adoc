...
. Specify headers which trigger the health check, if any. You can use link:./expression-language.html[Gravitee Expression Language^] to configure a header. Available variables are link:./expression-language.html#dictionaries[dictionaries^] and link:./expression-language.html#properties[api properties^] access.
