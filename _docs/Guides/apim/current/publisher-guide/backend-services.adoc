---
redirect_from:
  - /apim/3.x/apim_publisherguide_backend_services.html
---
...
. Specify headers which trigger the health check, if any. You can use link:/Reference/expression-language.html[Gravitee Expression Language^] to configure a header. Available variables are link:/Reference/expression-language.html#dictionaries[dictionaries^] and link:/Reference/expression-language.html#properties[api properties^] access.
