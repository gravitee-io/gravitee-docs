...
For a quick introduction to creating an API in APIM, see the link:/Getstarted/APIM/api-publisher-ui.html[Quick Start Guide^].
...
APIM Console provides various ways to create a new API. You can create a new definition from scratch or import an existing definition. You can also create a new API with APIM API, as described in the link:/Getstarted/APIM/api-publisher-api.html[Quick Start Guide^].
...
NOTE: You can choose to define your API plan (which your consumers will need before they can subscribe to your API) later. link:./plans/plans-subscriptions.html[Plans and subscriptions^] gives a detailed description of how to configure a plan and the different options available.
...
REQUEST only +
More information is available link:/Reference/policy/policy-rest-soap.html[here^].
...
== Publish your API and manage members

The API is created as a private API, so it is only accessible in APIM Portal to users after you:
