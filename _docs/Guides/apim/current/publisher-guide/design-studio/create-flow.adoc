---
redirect_from:
  - /apim/3.x/apim_publisherguide_design_studio_create.html
---
...
* One or more conditions to filter on, in link:/Reference/expression-language.html[Expression Language format^] -- for example, a condition to filter on query parameters called `X-debug` would be written like this: `#request.params['X-debug'] != null`
