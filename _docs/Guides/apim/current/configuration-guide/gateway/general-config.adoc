---
redirect_from:
  - /apim/3.x/apim_installguide_gateway_configuration.html
---
...
link:../introduction.html#configuring_apim_gateway_and_apim_api[Learn more about the different ways you can configure APIM Gateway^]
...
* request: Client authentication is not required but can be if using link:/Reference/Policy/policy-ssl-enforcement.html[SSL enforcement policy]
...
When defining the link:/Reference/policy/policy-rate-limiting.html[rate-limiting policy], APIM Gateway needs to store data to share with other APIM Gateway instances.
...
NOTE: If you want to use a custom prefix, you need to use the following link:../repositories/jdbc.html#use_a_custom_prefix[instructions^].
...
NOTE: If you want to use a custom prefix, you need to use the following link:../repositories/configure-repositories-jdbc.html#use_a_custom_prefix[instructions^].
