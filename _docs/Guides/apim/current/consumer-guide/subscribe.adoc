---
redirect_from:
  - /apim/3.x/apim_consumerguide_subscribe.html
---
= Subscribe to an API
...
To access the APIs and start interacting with them, consumers must first subscribe to an link:../publisher-guide/plans/create-plan.html[API plan^] with their registered application.

NOTE: Consumers must have a link:./create-account.html[user account] to register an application and subscribe to an API.

== Plan security type

Depending on the plan security type (API-Key, OAuth 2.0, JWT), you need to set proper HTTP headers to call the API (for more details, see link:../publisher-guide/plans/create-plan.html[Create a plan^].
...
For more information, see link:/Getstarted/apim/api-consume-ui.html[Subscribe to an API with APIM Portal^].
...
For each API you are subscribed to, APIM provides some analytics to show API usage in your application. For a list of the analytics available, see link:./manage-applications.html#analytics[Analytics^].
