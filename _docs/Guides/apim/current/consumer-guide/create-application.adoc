---
redirect_from:
  - /apim/3.x/apim_consumerguide_manage_applications.html
---
= Policies
...
NOTE: Only users with the required permissions can create an application. For more details, see the link:../administration-guide/roles-permissions.html[Roles and permissions^] section of the Administration Guide.
...
. (Optional) Search for an API and request a subscription (see link:/Getstarted/apim/consume-ui.html[Subscribe to an API with APIM Portal^]) in Getting Started. Click *NEXT*.
