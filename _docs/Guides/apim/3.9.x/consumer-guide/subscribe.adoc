...
Depending on the plan security type (API-Key, OAuth 2.0, JWT), you need to set proper HTTP headers to call the API (for more details, see ../publisher-guide/plans-subscriptions.html#create-a-plan[Create a plan^]).
...
For each API you are subscribed to, APIM provides some analytics to show API usage in your application. For a list of the analytics available, see link:../publisher-guide/logging-analytics.html[Analytics^].
