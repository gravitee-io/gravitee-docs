...
A plan provides a service and access layer on top of your API for consumer link:../introduction.html#application[applications].
...
To subscribe to an API, developers (consumers) must create an application and associate it with a plan.
This creates a subscription, which APIM uses to decide whether to accept or deny an incoming request.

You can find more information about applications and subscriptions in the link:../consumer-guide/introduction.html[API Consumer Guide^].
...
NOTE: From APIM 3.5.x, adding policies is taken care of as part of the API design process in link:./design-studio/introduction.html[Design Studio^].
...
Once you create and publish a plan it appears in Design Studio, where you can configure flow details for it. For more details, see link:./design-studio/create-flow.html[Design your API with Design Studio^].
...
image::apim/3.x/api-publisher-guide/plans-subscriptions/create-plan-path-authorization.png[Gravitee.io - Create plan - Resource Filtering, 873, 530, align=center, title-align=center]
