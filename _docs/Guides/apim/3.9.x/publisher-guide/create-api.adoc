...
For a quick introduction to creating an API in APIM, see the link:/Getstarted/quickstart-apim/api-publisher-ui.html[Quick Start Guide^].
...
APIM Console provides various ways to create a new API. You can create a new definition from scratch or import an existing definition. You can also create a new API with APIM API, as described in the link:/Getstarted/quickstart-apim/api-publisher-api.html[Quick Start Guide^].
