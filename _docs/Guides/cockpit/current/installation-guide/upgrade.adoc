...
Most of the time, data migration will be handled automatically thanks to a Liquibase script, so no manual work is required.
...
For users running Cockpit on K8S, the Liquibase migration may take too much time to execute, and K8S might kill the pod. If this happens, you should purge data using the following script before deploying the new version.
```
