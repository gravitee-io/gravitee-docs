...
To access theses APIs consumers, must subscribe to the link:/apim_publisherguide_plans_subscriptions.html[APIs' plans] with their registered application to start playing with the APIs.

NOTE: Consumer must have a link:/apim_consumerguide_create_account.html[user account] to register an application and subscribe to an API.
...
NOTE: Only users with correct role permission can create an application, see link:/apim_adminguide_roles_and_permissions.html[roles and permissions]
...
API's plans allow API publisher to provide and regulate access to their APIs. In order to use APIs you must subscribe to an API's plan, see link:/apim_publisherguide_plans_subscriptions.html[plans and subscriptions].
...
.Subscribe to an API
image::apim/api-consumer-guide/developer-subscribe/subscribe-to-an-api.png[Gravitee.io - subscribe to an api]
