...
APIM Console provides various ways to create a new API. You can create a new definition from scratch or import an existing definition.

All new API definitions start with clicking the plus icon image:apim/3.x/icons/plus-icon.png[] at the bottom of the *APIs* page:
...
For a quick introduction to creating an API in APIM, see the link:/apim/3.x/apim_quickstart_publish.html[Quick Start Guide^].
...
The value of this field is an `object` that follows this link:https://raw.githubusercontent.com/gravitee-io/gravitee-management-rest-api/master/gravitee-rest-api-service/src/main/resources/schema/xGraviteeIODefinition.json[JSON Schema^]
