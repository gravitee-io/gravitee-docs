...
TIP: An alternative method is to use `equals` instead of `==` - for example, if you are using an older version of APIM that may throw an error with a construct like in the example above. When you use `equals`, ensure that you put the string first - for example: `'my-value'.equals(#request.headers['my-header'][0])`.
