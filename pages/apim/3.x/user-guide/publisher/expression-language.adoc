...
.^|params        |Query parameters       ^.^|key / value     |order -> 100 (Both syntax works : `{#request.params.my-params}` and `{#request.params["my-params"]}`))
.^|pathParams    |Path parameters        ^.^|key / value     |storeId -> MyStore (__see Warning for details__)
.^|headers       |Headers                ^.^|key / value     |X-Custom -> myvalue (Both syntax works : `{#request.content.my-header}` and `{#request.content["my-header"]}`))
...
Expression language can also be used to set condition of execution [see conditional policies & flows conditions] and it is possible use logical operator as `&&` or `||` :

`{#request.headers['my-header'] != null && #request.headers['my-header'][0].equals("my-value")}`
