...
We are now going to use the API to `POST` data to Kafka over HTTP, and `GET` data from Kafka over HTTP. So that we can see what is happening on the MQTT side, we are going to use AKHQ.
