...
Over time, all policies will be migrated to support the event-native execution engine mode. The migration will ensure that all policies are backward compatible with the V3 execution mode throughout the V3 mode's normal product support life cycle.

=== Event-native mode

The new event-native execution mode comes with a new Policy interface, which allows you to execute all existing V3-mode policies without the need for any changes.

All policies related to security have already been migrated to support both V3 and event-native modes, as follows:
...
Custom policies developed by community users or customers should be perfectly compatible with the new mode, however we strongly recommend switching to an event-native mode implementation (a developer guide will be published soon).
...
In the new event-native execution mode, values of 0 and less are treated as meaning 'no timeout' (like in V3 mode). If you configure the timeout with a positive value, then it will act normally.
...
The parsed plan is selected for execution if all of the following conditions are met:
...
In V3 mode, Oauth2 plan *is selected* even if the incoming request does not match a subscription.
...
In event-native execution mode, Oauth2 plan *is not selected* if the incoming request does not match a subscription.

During the Oauth2 plan selection, a token introspection is released in order to retrieve the `client_id`, which allows to search for a subscription.

Due to concerns about performance, a cache system is available to avoid doing the same token introspection mutliple times.

If possible, we encourage using selection rules if there are mutliple Oauth2 plans, in order to avoid any unnecessary token introspection.
