...
The APIM Console use this default config :
```
...
```
The APIM Portal use this default config :
```
    add_header X-XSS-Protection "1; mode=block";
    add_header X-Content-Type-Options nosniff;
    add_header X-Permitted-Cross-Domain-Policies none;
...
It is recommended to complete this mechanisms to have a better control on the resources the user agent is allowed to load for a given page.


For portal you can improve security to allow specific origin, with this header :

```
add_header X-Frame-Options "ALLOW-FROM my-domain.com"
```
NOTE: APIM console uses an iframe to preview the portal theme configuration, so it is necessary to add the console in ALLOW-FROM domains in Portal config.


It is recommended to configure this header too : link:{{'https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy'}}[ Content-Security-Policy]
