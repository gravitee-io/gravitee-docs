...
:page-folder: am/protocols/oauth2
...
1. The user must be logged in to access his data. He asks the web application to sign in
2. The web application sends an Authorization Request (resource owner ask to grant access to his data) to the Authorization Server
...
[start=4]
4. The resource owner is authenticated and approved webapp acting on his behalf. The web app can ask for an access token
...
[start=5]
5. The web application just got an access token, it can use it to get user personal data
...
[start=6]
6. The Users API must check the incoming token to determine the active state of the access token and decide to accept or deny the request
...
[start=7]
7. The access is valid and the web application can display to the resource owner his personal data

8. If the resource owner decides to log out, the web application can ask Authorization Server to revoke active access token
