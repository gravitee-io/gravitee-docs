...
In order for an OpenID Connect Relying Party (client) to use openid services, it need first to register and be known by the OpenID Provider.
Dynamic Client Registration enable to self register RP by providing informations and obtain as a result the required informations (client_id) to use it.
Gravitee Access Management follow the link:https://openid.net/specs/openid-connect-registration-1_0.html[Openid Connect Dynamic Client Registration] specifications.
