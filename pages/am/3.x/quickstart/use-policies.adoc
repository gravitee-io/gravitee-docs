...
This section walks you through using flows to enhance the information displayed in the link:/am/current/am_userguide_user_management_consent.html[End user agreement^] by calling a remote service before rendering the HTML page. For a more detailed overview, see link:/am/current/am_userguide_policies.html[Flows^] in the User Guide.
The example in this section uses the Gravitee.io Echo API.

NOTE: AM Flows are available from version 3.5 and replace the AM extension points.
...
. Select the *CONSENT* flow and drag & drop the *HTTP Callout* policy into the *Pre Consent* step.
