...
WARNING: If you are facing to some certifiate issues with WebAuthn, please remember to upload on AM the last version of root certificate provided by your device producer as explain below in this page (Since AM 3.10).
...
WebAuthn relies on certificates to authenticate the device, these certificates may expire. If you are facing to some certifiate issues, please remember to check and to upload on AM the last version of root certificate provided by your device producer. Certificates can be uploaded on the WebAuthn settings page.

NOTE: Certificates upload is available since AM 3.10.

. link:/am/current/am_userguide_authentication.html[Log in to AM Console^].
. Select your *Security Domain*
. In the *Settings* menu, click *WebAuthn* in the *Security* section
+
