...
[start=3]
. *Identity Provider session* : if you application is configured to use a link:/am/current/am_userguide_social_identity_providers.html[social identity provider] to authenticate a user, the identity provider will create a session in addition of the AM one.
