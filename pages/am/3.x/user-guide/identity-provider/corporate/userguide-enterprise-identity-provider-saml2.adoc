...
[NOTE]
====
As the size of a SAML Response may be important you may have an error message in the Gateway logs like: *Size exceed allowed maximum capacity*. In such case, you have to adapt the *http.maxFormAttributeSize* value in the gravitee.yaml file (-1 for infinite)
====