...
Starting from AM 3.20, the certificate generated during the domain creation is marked as a "system" certificate. This is useful for the new "Certificate Rotation" feature that allow you to generate a new system certificate and assign this certificate to the applications that are using the previous system certicate.

When the Management API of AM 3.20 will start for the first time, the upgrader process will try to mark existing certificates as "system" based on the certificate name that should be "Default" and the creation date that should be the same as the domain creation date. If no system certificate can be determined, the domain and application will continue to work without issues.
