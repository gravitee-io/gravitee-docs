...
NOTE: settings for the email smtp server are based on the link:#configure_email[Configure email] section.

[source,yaml]
----
  notifier:
    enabled: false
    # when multiple Managemet REST API instances are deployed, 
    # enable this settings will introduce random number of seconds 
    # before each evaluation of a notification to try to avoid duplicates
    ...
    # Settings for Domain Certificate Expiry notifications
    certificates:
      # send notification if certificate is going to expire in less than 14 days
      expiryThreshold: 14
      # resend notification after 2 days
      resendAfter: 2
    email:
      enabled: true
      sslTrustAll: false
